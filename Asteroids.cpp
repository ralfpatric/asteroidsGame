...
Asteroids::Asteroids(const sre::Sprite &sprite) : GameObject(sprite) {
	scale = glm::vec2(0.5f, 0.5f);
	winSize = sre::Renderer::instance->getWindowSize();
	radius = 23;
	position = glm::vec2(winSize.x * 0.2f, winSize.y * 0.4f);
...
void Asteroids::onCollision(std::shared_ptr<GameObject> other) {
	// Add asteroids splits here on collision
}

void Asteroids::onKey(SDL_Event &keyEvent) {
